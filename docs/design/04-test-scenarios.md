...
| K7 | `SetOverlayThresholdsAsync(w, c)` 后重读；四种方向组合（都升 / 都降 / 一升一降 ×2） | `WarningThresholdMb == w && CriticalThresholdMb == c`（阈值是否需重启 `[待 VM 确认]`——若需，断言改到重启后）；Mock 断言两次内部调用的顺序满足每步 warning < critical |
