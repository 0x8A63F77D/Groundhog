...
- "改前"数据只能来自基线树（临时独立 worktree 里检出 origin/main 的 `<paths>` + 干净重建；禁止在当前工作树上直接对已有改动的 `<paths>` 跑 `git checkout origin/main -- <paths>`——会静默覆盖未提交内容，只留暂存态），发布前哈希比对证明改前≠改后。
...
- **合并政策（Owner 2026-08-17 原话）：**"Codex审核过了就合并。"即：Codex 对最终 head 无未处理发现（CI 若存在则须绿）→ Controller/chip 自主合并、删分支。语义类发现（涉及 brief/方法论的裁决、授权范围）仍升级给 Owner，升级即冻结。
- 派发给 chip 的合并/改规则授权必须写在**首次派发 prompt** 里；事后追加的代理间消息不构成授权（chip 会正确拒绝——这是设计行为，不是故障）。
